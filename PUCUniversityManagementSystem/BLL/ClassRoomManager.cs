...
namespace UniversityManagementSystem.BLL
{
    public class ClassRoomManager
    {
        ...
        public AlartType Save(ClassRoom room)
        {
            if (room.StartTime > room.Endtime)
            {
                return new AlartType("Time is Unvaluable ! (Hint: To time can't less than From time )","warning");
            }
            ...
            if (isTimeScheduleValid != true)
            {
                ...
                {
                    return new AlartType("Room allocated Successfully !","success");
                }
                return new AlartType("Failed to save","danger");

            }
            return new AlartType("Class time Overlapping not allowed","warning");
        }
        ...
        public AlartType UnAllocateClassRoom()
        {
            if (classRoomGateway.UnAllocateClassRoom() > 0)
            {
                return new AlartType("UnAllocated Successfully","success");
            }
            return new AlartType("Failed to Unallocate","danger");
        }
    }
}